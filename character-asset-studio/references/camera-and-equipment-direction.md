...
Before generating a pose, pair a locked character version with locked equipment versions. The fusion manifest's `actionPlan.equipmentInteractions` must contain one entry per retained tool. For an active tool record `assetId`, `state`, `hand`, `gripPoint`, `bodyContact`, `lineOfAction`, and `clearance`; for a stowed tool record `assetId`, `state`, `attachmentAnchor`, `bodyContact`, and `clearance`. Keep these states equal to the equipment ledger and use the spec's permitted actions. The production validator rejects missing or contradictory entries. Define an interaction record for **every** item: equipment ID/version, apparent scale in head units, active or stowed state, selected permitted action, dominant/support hand, grip anchors, waist/back attachment if stowed, line of force, target, clearance, and visibility. Choose the pose from this record rather than decorating a completed pose with tools. When an action changes, retain size/shape/color/style locks and update only state, contacts, body mechanics, and allowed flexible movement.
