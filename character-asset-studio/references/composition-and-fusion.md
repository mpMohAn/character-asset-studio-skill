...
2. Assign a source ID to every output component. Name the exact feature inherited, including the face/expression, hair, body, each garment, and every tool. Use one source for the underlying face and body anatomy unless the user specifically requests a different split. For a selected fusion, map the observed neck, shoulder, chest, torso, abdomen, waist, hip, arm, and leg shapes to that body source, or document an explicit source per region. Preserve only source-visible abdominal definition and garment underlayers; unknown or occluded parts remain unasserted. Define the action's cast/contact shadow from its light and support, not by copying a permanent shadow onto the body. A recognizable contribution from every source must survive; do not rely on a color or tiny ornament as the sole contribution.
...
After rendering, compare the generated figure side by side with the individual sources at a similar apparent size. Complete every `review` field with an observation. Run `python3 scripts/validate_asset_locks.py path/to/fusion.json --final` before approving a master. This final gate requires evidence notes and a pass for every review area; **reject the draft** if any one fails:
